...
impl Datasource for RpcTransactionAnalyzer {
  ...
  fn update_types(&self) -> Vec<UpdateType> { vec![UpdateType::Transaction] }
}
...
fn transaction_fetcher(
  ...
  tokio::spawn(async move {
    ...
    tokio::select! {
        _ = cancellation_token.cancelled() => {
            info!("Cancelling RPC Crawler transaction fetcher...");
        }
        _ = fetch_stream_task => {}
    }
  })
...
fn task_processor(
  ...
  tokio::spawn(async move {
    loop {
      tokio::select! {
          _ = cancellation_token.cancelled() => {
              info!("Cancelling RPC Crawler task processor...");
              break;
          }
          Some((signature, fetched_transaction)) = transaction_receiver.recv() => {
              ...
              let meta_original = if let Some(meta) = transaction.clone().meta {
                  ...
                  warn!("Meta is malformed for transaction: {:?}", signature);
                  continue;
              };
              ...
              let Some(decoded_transaction) = transaction.transaction.decode() else {
                  error!("Failed to decode transaction: {:?}", transaction);
                  continue;
              };
              ...
              let Ok(meta_needed) = transaction_metadata_from_original_meta(meta_original) else {
                  error!("Error getting metadata from transaction original meta.");
                  continue;
              };
