...
impl CreatorInstructionProcessor {
    pub fn new(mint_address: Pubkey, creator_handler: Arc<CreatorHandlerOperator>, cancellation_token: CancellationToken) -> Self {
    ...
    pub fn set_creator(&mut self, analyzed_account: Pubkey) {
    ...
    async fn process_sender(&mut self, sender: Pubkey, receiver: Pubkey, amount: f64, timestamp: i64) -> Option<(Cex, Vec<Pubkey>)> {
  ...
  async fn process(
      ...
      SystemProgramInstruction::TransferSol(transfer_sol) => {
        ...
        if let Some(accounts) = accounts {
            if amount > 0.85 && accounts.source != self.analyzed_account {
                ...
                let timestamp = meta.transaction_metadata.block_time.unwrap_or(chrono::Utc::now().timestamp_millis());
                
                // Process this sender
                if let Some((cex, path)) = self.process_sender(
                    accounts.source, 
                    accounts.destination, 
                    amount, 
                    timestamp
                ).await {
                    info!("cex_connection::{}:{} (path length: {})", 
                         self.mint_address, cex.name, path.len());
                    
                    // Notify handler about CEX connection
                    if let Err(e) = self.creator_handler.record_cex_connection(
                        cex,
    ...
    if let Some((address, depth, _)) = self.queue.pop_front() {
        ...
        if address != self.analyzed_account {
            ...
            if let Err(e) = self.creator_handler.process_next_bfs_level(
                address,
                depth,
                self.mint_address,
                self.connection_graph.clone_graph()
