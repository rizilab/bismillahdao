    ...
    loop {
        tokio::select! {
            ...
            _ = token_creation_metadata.shutdown.wait_for_shutdown() => {
                debug!("token_creation_metadata::received_shutdown_signal");
                break;
            },
            else => {
                debug!("token_creation_metadata::all_senders_dropped");
                break;
            }
        }
    }
...
impl TokenHandlerMetadataOperator {
    ...
    pub async fn store_token(
        ...
        match self.sender.try_send(TokenHandler::StoreToken {
            token_metadata,
        }) {
            Ok(()) => {
